...
# ╔═╡ 7b1a3ebc-f7ca-11ec-3c21-958ccba517e2
begin
	using StatsPlots
	using JLD
	using Distributions
...
# ╔═╡ 5bf0131c-2bb2-46be-8ecf-70cdb575db95
begin
	# t_samp  = 1e-3
	# pulse = load("./dispersed_pulse.npz")
	# dt = size(pulse)[1] * t_samp
	# println(dt)
 ...
 #    savefig(dispersed_pulse_plot, "./reports/dispersed_pulse_plot.png")
	# dispersed_pulse_plot
end

# ╔═╡ 4a7418ad-0530-435b-88b6-eced7bef7246
begin
	local dict = load("./reports/data/dedisp_outputs.jld")
	pyfdmt_out = dict["pyfdmt"]
    fdmt_jl_out = dict["fdmt"]
    dedisp_cpu_out = dict["dedisp_cpu"]
    dedisp_gpu_out = dict["dedisp_gpu"]
	fdmt_gpu_out = load("./reports/fdmt_gpu_out.npy")
	nothing
end

# ╔═╡ 3e8ad040-92af-48d6-b32d-a4be7290bd23
dict = load("./bench_times.jld")

# ╔═╡ bf8240f8-3605-4bd7-b52f-b4cd1cd672a3
begin
	pyfdmt_times = dict["pyfdmt"]
	fdmt_cpu_times = dict["fdmt"]
	dedisp_cpu_times = dict["dedisp_cpu"]
	dedisp_gpu_times = dict["dedisp_gpu"]
	fdmt_gpu_times = load("./reports/fdmt_gpu_times.npy")
	nothing
end

# ╔═╡ 2327d2e7-7d54-43cb-ab57-7d0677bf8a3a
begin
	labels = ["Brute-force CPU\nJulia\nN=$(length(dedisp_cpu_times))", 
				"Brute-force GPU\nJulia\nN=$(length(dedisp_gpu_times))",
				"FDMT CPU\nPython\nN=$(length(pyfdmt_times))",
				"FDMT CPU\nJulia\nN=$(length(fdmt_cpu_times))", 
				"FDMT GPU\nPython\nN=$(length(fdmt_gpu_times))"
				]
	
	grouped_times = [median(dedisp_cpu_times) ;
	                 median(dedisp_gpu_times) ;
					 median(pyfdmt_times)     ;
	                 median(fdmt_cpu_times)   ;
	                 median(fdmt_gpu_times)   ;
	                 ]
	
	ctg = repeat(["Mean", "Min"], inner = 5)
	
	runtime_plot = bar(labels,
							grouped_times,
							yaxis=(:log10, (collect(10 .^ (-3.0,-2.0,-1.0,0,1,2)))),
							ytickfont = font(10, "Courier"),
							legend=false,
							# group=ctg,
							ylabel="Median Runtime in Seconds",
...
# ╔═╡ c972ad69-21d6-4cfd-9871-091259be4599
begin
	relative_labels = [#"Brute-force CPU\nJulia\nN=$(length(dedisp_cpu_times))", 
				"FDMT CPU\nPython\nN=$(length(pyfdmt_times))",
				"FDMT CPU\nJulia\nN=$(length(fdmt_cpu_times))", 
				"Brute-force GPU\nJulia\nN=$(length(dedisp_gpu_times))",
				"FDMT GPU\nPython\nN=$(length(fdmt_gpu_times))"
	...
	relative_times = [#median(dedisp_cpu_times) ;
					 median(pyfdmt_times)     ;
	                 median(fdmt_cpu_times)   ;
					 median(dedisp_gpu_times) ;
	                 median(fdmt_gpu_times)   ;
	                 ]
	relative_times = median(dedisp_cpu_times) ./ relative_times
	
	relative_runtime_plot = bar(relative_labels,
			log10.(relative_times),
			# yaxis=(:log, (1,100000)),
			ylim=(0,6), 
			yticks=((0:6), ("$(10^i)" for i in 0:6)),
			legend=false,
			ylabel="Speedup Factor Compared to Brute-force CPU",
			xlabel="Implementation",
			title="Dedispersion Runtime Comparison")
	plot!(size=(700,600))
	savefig(relative_runtime_plot, "./reports/dedisp_relative_runtimes_plot.png")
	relative_runtime_plot
end

# ╔═╡ 7217a7cf-8c4d-408f-8904-6fa3a5ff2eb5
begin
	fdmt_gpu_plot = heatmap(fdmt_gpu_out,
							title="Bifrost FDMT GPU Output",
							ylabel="DM",
